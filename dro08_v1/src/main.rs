...
mod app {
    ...
    #[task(
        ...
        local = [relay,encoder],
        shared = [
            scale_factor, scaled_value,limit_1,preset_count,
            limit_2, relay_time,reset_requested, preset_requested, rl1_active, rl2_active
        ]
    )]
    fn relay_task(mut ctx: relay_task::Context) {
        ...
        let preset = ctx.shared.preset_count.lock(|p| *p);

        // if reset then reset encoder
        ...
        // if preset than preset encoder
        let preset_pressed = ctx.shared.preset_requested.lock(|r| {
            let val = *r;
            *r = false;
            val
        });
        ...
        if preset_pressed {
            encoder.preset(scale.unapply(preset));
        };

        // Read count from encoder and scale_factor from shared
        let count = encoder.update_and_read_i32();
        let scale_val = scale.apply(count);
    ...
    #[task(
        ...
        // Cleaned: removed reset_requested since it's no longer used here
    ...
    shared = [
        ...
        reset_requested, preset_requested
    ]
)]
    fn normal_task(mut ctx: normal_task::Context) {
        ...
        match action {
            dro08::DisplayAction::ApplyPreset => {
                ctx.shared.preset_requested.lock(|r| *r = true);
            }
